...
const BrandIntroduction = () => {
  ...
  return (
    <main className="relative   text-white/90 px-4 py-12 sm:px-6 lg:px-8">
      ...
      <div className="mt-16 grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-4 max-w-6xl mx-auto">
        {benefits.map((benefit) => (
          <div key={benefit.title} className="bg-white/10 rounded-2xl p-6 shadow-md hover:bg-white/20 transition">
            <h3 className="text-xl font-semibold text-green-500">{benefit.title}</h3>
            <p className="mt-2 text-gray-300 text-sm">{benefit.description}</p>
          </div>
        ))}
      </div>
      ...
      <div className="mt-20 max-w-5xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-8 items-center">
        <div>
          <h2 className="text-3xl sm:text-4xl font-bold">Чому обирають нас?</h2>
