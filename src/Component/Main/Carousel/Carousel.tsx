            ...
            <div className="relative">
              ...
              >
                <img
                  ...
                  className="w-full h-[400px] md:h-[400px] object-cover rounded-3xl"
                  loading="lazy"
                />
                <InfoCard activeSlide={activeSlide} index={index}>
                  ...
                  <Link to={`/product/${product.id}`}>
                    <button className="rounded-3xl border-2  backdrop-blur-md px-4 py-2  shadow-sm transition-transform hover:bg-black/50 hover: hover:scale-[105%] active:scale-[95%] duration-300 ease-in">
                      Детальніше
                    </button>
                  </Link>
                </InfoCard>
...
const InfoCard: React.FC<InfoCardProps> = ({ activeSlide, index, children }) => {
  return (
    <div
      className={`absolute bottom-0 left-0 right-0 p-6 bg-black/30 backdrop-blur-md  rounded-3xl transition-all duration-500 ${
        activeSlide === index ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"
      }`}
    >
      {children}
    </div>
